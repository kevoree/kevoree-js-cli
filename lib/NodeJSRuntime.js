...
NodeJSRuntime.prototype.start = function (nodename, groupname, groupport, loglevel) {
	...
	var rl = process.platform === 'win32' ? readline.createInterface({
...
NodeJSRuntime.prototype.deploy = function (model) {
	...
	bootstrapHelper(this.kevs, options, function (err, bootstrapModel) {
		if (err) {
			...
			var node = bootstrapModel.findNodesByID(options.nodeName);
			...
			this.kCore.deploy(bootstrapModel, function (err) {
				if (err) {
					try {
						...
						var currModelPath = path.join(os.tmpdir(), 'model-' + this.nodename + '-' + timestamp + '.curr.json');
						var deplModelPath = path.join(os.tmpdir(), 'model-' + this.nodename + '-' + timestamp + '.depl.json');
						var factory = new kevoree.factory.DefaultKevoreeFactory();
						var serializer = factory.createJSONSerializer();
						var currModelStr = JSON.stringify(JSON.parse(serializer.serialize(this.kCore.currentModel)), null, 2);
						var deplModelStr = JSON.stringify(JSON.parse(serializer.serialize(bootstrapModel)), null, 2);
